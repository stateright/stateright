    ...
    fn init_states(&self) -> Vec<Self::State> {
        vec![TwoPhaseState {
            rm_state: self
                .rms
                .iter()
                .map(|rm| (rm.clone(), RmState::Working))
                .collect(),
    ...
    fn next_state(&self, last_state: &Self::State, action: Self::Action) -> Option<Self::State> {
        ...
        match action.clone() {
            ...
            Action::RmChooseToAbort(rm) => {
                state.rm_state.insert(rm, RmState::Aborted);
            }
            Action::RmRcvCommitMsg(rm) => {
                state.rm_state.insert(rm, RmState::Committed);
            }
