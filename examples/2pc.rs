...
fn can_model_2pc() {
    ...
    // reverify the smaller state space with symmetry reduction
    let checker = TwoPhaseSys { rms: 0..3 }.checker().symmetry().spawn_bfs().join();
    assert_eq!(checker.unique_state_count(), 120);
    checker.assert_properties();
