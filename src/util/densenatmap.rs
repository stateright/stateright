...
mod test {
    ...
    pub fn can_rewrite() {
        ...
        impl Rewrite<Id2> for Id {
            fn rewrite<S>(&self, _: &RewritePlan<Id2, S>) -> Self {
                *self
            }
        }
        ...
        assert_eq!(
            f3.rewrite(&plan),
            DenseNatMap::from_iter(['X', 'Y', 'W', 'Z'])
        ); // *not* reindexed
        assert_eq!(f4.rewrite(&plan), BTreeMap::from_iter([('!', Id::from(1))]));
        assert_eq!(f5.rewrite(&plan), BTreeMap::from_iter([(Id::from(1), '!')]));

        // Rewriting based on `Id2` on the other hand impacts *only* the third field.
        let plan = RewritePlan::from(&f3);
        assert_eq!(
            f1.rewrite(&plan),
            DenseNatMap::from_iter(['B', 'C', 'A', 'D'])
        ); // *not* reindexed
        assert_eq!(
            f2.rewrite(&plan),
            DenseNatMap::from_iter(['X', 'Y', 'W', 'Z'])
        ...
        assert_eq!(f4.rewrite(&plan), BTreeMap::from_iter([('!', Id::from(0))])); // Id is *not* rewritten
        assert_eq!(f5.rewrite(&plan), BTreeMap::from_iter([(Id::from(0), '!')]));
        // Id is *not* rewritten
    }
}
